...
const CACHE_KEY = "browse_anime_cache";
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const listUrl = `${API_BASE}/api/anime`;
        ...
        const detailPairs = await Promise.all(
          detailTargets.map(async (item) => {
            try {
              const detailUrl = `${API_BASE}/api/anime/${item.id}/episodes`;
    ...
    <main className="app-container">
      <div className="browse-page">
        <div className="home-browse-links">
          <a href="#/manga" className="browse-link-btn">📚 Truyện</a>
          <a href="#/anime" className="browse-link-btn active">🎬 Anime</a>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-block">Chưa có anime nào.</div>
        ) : (
          <div className="browse-grid">
              ...
              return (
                <a key={item.id} href={item.primaryHref} className="browse-card">
                  ...
                  <div className="browse-meta">
                    <span>{firstRow?.left || `${item.detailCount || 0} Tập`}</span>
